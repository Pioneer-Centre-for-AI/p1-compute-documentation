...
## How do I access services on the remote machine? <Badge text="NOT RELEVANT FOR AIRGAPPED SYSTEMS" variant="caution" />
...
### LSF Job Submission Script <Badge text="REMOTE" variant="note" />
...
#BSUB -gpu "num=1"             # GPU requirement
...
export const entrypointScript = `#!/bin/bash
...
wait $OLLAMA_PID $JUPYTER_PID`;

<Code code={entrypointScript} lang="bash" title="entrypoint.sh" />

### SSH Tunnel <Badge text="LOCAL" variant="note" />
...
ssh -L 11434:localhost:11434 -L 8888:localhost:8888 p1-hpc -N -f
